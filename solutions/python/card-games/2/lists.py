...
def get_rounds(number: int) -> list:
    ...
    """
    return [ card + number for card in range(3) ]
...
def approx_average_is_average(hand: list) -> bool:
    ...
    return true_avg in [hand[len(hand) // 2], card_average([hand[0], hand[-1]])]
...
def maybe_double_last(hand: list) -> list:
    ...
    """
    if hand[-1] == 11:
        return [ card * 2 if card == 11 else card for card in hand ]
    return hand
